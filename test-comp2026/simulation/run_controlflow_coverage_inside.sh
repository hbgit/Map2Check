#!/bin/bash
# run_controlflow_coverage_inside.sh — Run C.coverage-error-call.ControlFlow
# Follows TestComp 2026 guidelines
# Usage: This script runs INSIDE the Docker container via:
#   docker run --rm -v $(pwd):/workspace map2check-dev bash /workspace/test-comp2026/simulation/run_controlflow_coverage_inside.sh
set -e

MAP2CHECK_PATH=/workspace/test-comp2026/simulation/release
SV_BENCH=/workspace/test-comp2026/simulation/sv-benchmarks/c
RESULTS_DIR=/workspace/test-comp2026/simulation/resultados_de_testes/controlflow_coverage_error_call
TIMEOUT=300
...
CSV="$RESULTS_DIR/controlflow_coverage_results_v8.csv"
...
process_set() {
    ...
    while IFS= read -r line; do
        ...
        for yml in $SV_BENCH/$line; do
            ...
            # For coverage-error-call, check if program has reach_error
...
echo "Category: C.coverage-error-call.ControlFlow"
echo "============================================================"
echo "Timeout: ${TIMEOUT}s | Solver: z3 | Property: coverage-error-call"
...
echo ""
echo "============================================================"
echo "SUMMARY — C.coverage-error-call.ControlFlow"
echo "============================================================"
echo "Total tasks:    $TOTAL"
echo "FALSE (reach_error found): $T_FALSE"
echo "TRUE:           $T_TRUE"
