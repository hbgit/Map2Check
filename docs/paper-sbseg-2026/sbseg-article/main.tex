...
\begin{abstract}
  ...
  modernization in 2026: migration to LLVM~16, adoption of the New Pass Manager,
  C++17 standard, CI/CD infrastructure, static analysis, and sanitizers. This
  paper presents the engineering modernization, reports results from TestComp
  2026 (594 tasks, score~57), and discusses ongoing WebAssembly support.
\end{abstract}
...
\begin{resumo}
  ...
  para LLVM~16, adoção do New Pass Manager, padrão C++17, infraestrutura de
  CI/CD, análise estática e sanitizers. Este artigo apresenta a modernização
  de engenharia, reporta resultados do TestComp~2026 (594 tarefas, score~57)
...
Lista de contribuições: (1) migração LLVM~6$\rightarrow$16, New Pass
Manager, C++17; (2) infraestrutura CI/CD, Docker, sanitizers, análise
estática; (3) validação em benchmarks reais: TestComp~2026 (Heap, 594
tarefas, score~57); (4) fortalecimento em cibersegurança (mapeamento CWE);
...
timeout 300s, Z3, target \texttt{reach\_error}, Docker single-container.
Resultados em 594 tarefas~
:
...
TIMEOUT & 2 & 0,3 \% \\
\midrule
\textbf{Total} & \textbf{594} & \textbf{100 \%} \\
\textbf{Score} & \textbf{57} & --- \\
\bottomrule
\end{tabular}
\end{table}
...
Modernização: 35 commits, 84 arquivos, +5.785/$-$657 linhas. 9 passes
migrados, 7/7 unit tests, 9/9 passes carregando. TestComp~2026 Heap:
594 tarefas, score~57, 56 bugs reais.

\textbf{[PARÁGRAFO -- Limitações]}
Taxa de UNKNOWN alta (45,6\%). Ausência de testes E2E no CI.
