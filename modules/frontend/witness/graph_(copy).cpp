...
SVCompWitness::SVCompWitness(std::string programPath, std::string programHash,
  ...
  switch (violated.propertyViolated) {
    ...
    case Tools::PropertyViolated::TARGET_REACHED:
      specification = std::make_unique<Specification>(SpecificationType::TARGET,
                                                      targetFunction);
      this->automata = std::make_unique<ViolationWitnessGraph>();
      break;
        ...
        if ((trackBBLineNum >= stateTrueNumLineBeginBB) &&
          ...
          if (std::stoi(stateTrueLogRows[k].hasControlCode)) {
            ...
            std::unique_ptr<EdgeData> sourcecode =
                std::make_unique<SourceCode>(stateTrueLogRows[k].controlCode);
            newEdge->AddElement(std::move(sourcecode));

            // attribute control
            // search in trackBBLogRows which condition (TRUE or FALSE) by line
            // number in stateTrueLogRows was executed
            int tmpCount = i + 1;  // true cond
            bool hasTrueCond = false;

            if (tmpCount < trackBBLogRows.size()) {
              // cout << stateTrueLogRows[k].numLineControlTrue << " == " <<
              // trackBBLogRows[tmpCount].numLineInBB << "\n";
              if (std::stoi(stateTrueLogRows[k].numLineControlTrue) ==
                  std::stoi(trackBBLogRows[tmpCount].numLineInBB)) {
            ...
            if (tmpCount < trackBBLogRows.size()) {
              ...
              // cout << stateTrueLogRows[k].numLineControlFalse << "==" <<
              // trackBBLogRows[tmpCount].numLineInBB << "\n";
              if (std::stoi(stateTrueLogRows[k].numLineControlFalse) ==
                  std::stoi(trackBBLogRows[tmpCount].numLineInBB)) {
                ...
                // std::unique_ptr<Edge> newEdge =
                // std::make_unique<Edge>(lastStateId, cnvt.str());
                // std::unique_ptr<Node> newNodeF =
                // std::make_unique<Node>(cnvtF.str());
                // this->automata->AddNode(std::move(newNodeF));
                // Create the edge to the new node
                std::unique_ptr<Edge> newEdgeF =
                    std::make_unique<Edge>(tmpLastStateId, cnvtF.str());

                // attribute startline
                std::unique_ptr<EdgeData> startLineF =
                    std::make_unique<StartLine>(
                        std::to_string(stateTrueNumLineStart));
        ...
        if ((trackBBLineNum >= stateTrueNumLineBeginBB) &&
          ...
          if (std::stoi(stateTrueLogRows[k].hasControlCode)) {
            ...
            if (i < trackBBLogRows.size()) {
              cout << tmpi << "<<<<<<<<<<<< \n";
              cout << stateTrueLogRows[k].numLineControlTrue
                   << "==" << trackBBLogRows[tmpi].numLineInBB << "\n";
              if (std::stoi(stateTrueLogRows[k].numLineControlTrue) ==
               ...
               * */
                ...
                // attribute startline
                // std::unique_ptr<EdgeData> startLineF =
                // std::make_unique<StartLine>(std::to_string(stateTrueNumLineStart));
  ...
  if (kleeLogRows.size() == 0 && listLogRows.size() == 0 &&
    ...
    for (int i = 0; i < kleeLogRows.size(); i++) {
      ...
      std::unique_ptr<EdgeData> assumption =
          std::make_unique<AssumptionEdgeData>(value, "__VERIFIER_nondet_int",
                                               functionName);
