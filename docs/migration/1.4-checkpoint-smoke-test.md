...
### Bug 1: KLEE 3.1 CLI flags (commit `284aef1ac`)

**Symptom:** `klee: Unknown command line argument '-suppress-external-warnings'`

**Root Cause:** KLEE 3.1 (LLVM 16) removed several CLI flags that existed in
KLEE 2.x (LLVM 6):
...
**Fix:** Updated `caller.cpp` to use KLEE 3.1-compatible flags.
