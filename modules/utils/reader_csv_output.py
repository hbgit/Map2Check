...
class ReaderCsv(object):
    ...
    def loadCsvFile(self, pathCsvFile, _opdelimiter):
        ...
        #header_list=["#","Address","Address points to","Function ID","IS DYNAMIC","Set FREE","IS UNION","Line","Var Name","Var Value"]
        ...
        with open(pathCsvFile) as file:
            ...
            for row in reader:
                ...
                for (k,v) in row.items():
                    #self.columns[header_list[iheader]].append(v)
                    self.columns[k].append(v.strip())
                    #iheader += 1
